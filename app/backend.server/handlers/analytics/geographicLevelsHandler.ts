 ...
 */
export async function getGeographicLevelsHandler(
		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Failed to fetch geographic levels",
		};
	}
}
...
export async function getGeographicLevelsByLevelHandler(
	...
	try {
		...
		// Get geographic levels from model - pass countryAccountId as string
		const levels = await getGeographicLevelsByLevel(
			countryAccountsId.toString(),
			levelNumber,
		);
		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Failed to fetch geographic levels by level",
		};
	}
}
...
export async function getGeographicLevelByIdHandler(
	...
	try {
		...
		const level = await getGeographicLevelById(
			countryAccountsId.toString(),
			levelId,
		);
		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Failed to fetch geographic level",
		};
	}
}
