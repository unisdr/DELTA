        ...
        if (filters) {
            if (filters.startDate) {
                try {
                    ...
                    if (startDate) {
                        conditions.push(
                            createDateCondition(disasterRecordsTable.startDate, startDate, 'gte'),
                        );
        ...
        if (recordLossAmount === 0) {
            ...
            for (const loss of matchingLosses) {
                ...
                if (loss.publicCostTotalOverride) {
                    ...
                    lossAmount +=
                        (Number(loss.publicUnits) || 0) * (Number(loss.publicCostUnit) || 0);
                }
                ...
                if (loss.privateCostTotalOverride) {
                    ...
                    lossAmount +=
                        (Number(loss.privateUnits) || 0) * (Number(loss.privateCostUnit) || 0);
                }
    ...
    try {
        ...
        return {
            ...
            damageOverTime: Object.fromEntries(
                [...new Set([...eventCounts.keys(), ...damagesResult.byYear.keys()])].map(
                    (year) => [year.toString(), (damagesResult.byYear.get(year) || 0).toString()],
                ),
            ),
