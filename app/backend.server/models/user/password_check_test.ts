...
describe("checkPasswordComplexity", () => {
	...
	it("empty password", async () => {
		deepEqual(fn(""), {
			error: PasswordErrorType.Empty,
			characterClasses: new Set(),
		});
	});
	it("valid", async () => {
		deepEqual(fn("]ARi3w.kOi3j"), {
			error: null,
			characterClasses: new Set([
	...
	it("too short", async () => {
		deepEqual(fn("]ARi3w.kOi3"), {
			error: PasswordErrorType.TooShort,
			characterClasses: new Set([
				PasswordCharClass.Digit,
				PasswordCharClass.Lowercase,
				PasswordCharClass.Uppercase,
	...
	it("character classes", async () => {
		deepEqual(fn("123456789012"), {
			error: PasswordErrorType.InsufficientCharacterClasses,
			characterClasses: new Set([PasswordCharClass.Digit]),
		});
	});
});
