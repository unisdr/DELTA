export const initTokenField = (
		...
		if (typeof dataSource === "string") {
			...
			try {
				...
				if (response.ok) {
					...
					console.error(
						`Failed to fetch initial dataSource: ${response.statusText}`,
					);
				}
			} catch (error) {
	...
	if (!wrapper) {
		console.error(
			"Tokenfield initialization failed: No parent element found for input.",
		);
		return;
	}
	...
	let tokenContainer = wrapper.querySelector(
	...
	let dropdown = tokenContainer.querySelector(
		".custom-tokenfield-dropdown",
	) as HTMLElement;
	if (!dropdown) {
	...
	const selectedItems: { id: number; name: string }[] = initialValue
		? initialValue
		: [];
	...
	const updateDropdownHighlight = () => {
		const options = Array.from(
			dropdown.querySelectorAll(".custom-tokenfield-dropdown-item"),
		) as HTMLElement[];
		options.forEach((option, index) => {
			Object.assign(option.style, {
				background: index === highlightedIndex ? "#ddd" : "white",
			});
		});
	};
	...
	const handleInput = async () => {
		...
		if (fetchedData) {
			suggestions = fetchedData.filter(
				(item: any) => item.name.toLowerCase().includes(query), // Matches starting with query
			);
		} else {
		...
		if (e.key === "Enter") {
			...
			if (highlightedIndex >= 0 && options[highlightedIndex]) {
				const selectedItem = fetchedData
					? (fetchedData as { id: number; name: string }[]).find(
	...
	const enableDragAndDrop = () => {
		...
		const updateTokensForDrag = () => {
			const tokens = tokenContainer.querySelectorAll(
				".custom-token",
