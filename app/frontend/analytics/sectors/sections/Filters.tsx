...
const Filters: React.FC<FiltersProps> = ({
	...
	const handleFilterChange = (field: keyof typeof filters, value: string) => {
		...
		setFilters((prev) => {
			...
			if (field === "fromDate" && prev.toDate && value > prev.toDate) {
				...
				Swal.fire({
					icon: 'warning',
					text: ctx.t({ "code": "common.from_date_later_than_to_date", "msg": "From date is later than To date. The To date has been cleared." }),
					confirmButtonText: 'OK',
					buttonsStyling: false,
					customClass: {
						popup: 'swal2-custom-popup',
	...
	const handleApplyFilters = () => {
		...
		if (filters.fromDate && filters.toDate && filters.fromDate > filters.toDate) {
			...
			Swal.fire({
				icon: 'warning',
				text: ctx.t({ "code": "common.from_date_cannot_be_later_than_to_date", "msg": "From date cannot be later than To date. Please adjust your date selection." }),
				confirmButtonText: 'OK',
				buttonsStyling: false,
				customClass: {
					popup: 'swal2-custom-popup',
		...
		return (
			<>
				...
				<div style={{ position: "relative" }}>
					...
					{loading && <p style={{ color: "blue", fontStyle: "italic" }}>Loading...</p>}
						...
						<ul className="autocomplete-list">
							{filteredItems.length > 0 ? (
								...
								<li className="no-results">
									{ctx.t({ "code": "common.no_match_found", "msg": "No match found" })}
								</li>
							)}
						</ul>
					)}
	...
	return (
		<div className="mg-grid mg-grid__col-6">
			...
			<div className="dts-form-component mg-grid__col--span-2">
				...
				<label>
					<div className="dts-form-component__label">
						<span>{ctx.t({ "code": "common.from_date", "desc": "From date", "msg": "From" })}</span>
					</div>
					<input
						type="date"
						id="from-date"
			...
			<div className="dts-form-component mg-grid__col--span-2">
				...
				<label>
					<div className="dts-form-component__label">
						<span>{ctx.t({ "code": "common.to_date", "desc": "To date", "msg": "To" })}</span>
					</div>
					<input
						type="date"
						id="to-date"
			...
			<div className="dts-form-component mg-grid__col--span-4">
				...
				<div style={{ position: "relative" }}>
					...
					<input
						id="event-search"
						aria-label={ctx.t({ "code": "analysis.search_disaster_events", "msg": "Search for disaster events" })}
						type="text"
						className="filter-search"
						placeholder={ctx.t({ "code": "disaster_event.search_by_name_id_glide_placeholder", "msg": "Type to search by name, ID, GLIDE number..." })}
						...
						filters.disasterEventId && showResults && (
							<ul className="autocomplete-list">
								{filteredEvents.length > 0 ? (
									...
									<li className="no-results">
										{ctx.t({ "code": "analysis.no_matching_events_found", "msg": "No matching events found" })}
									</li>
								)}
							</ul>
						)
