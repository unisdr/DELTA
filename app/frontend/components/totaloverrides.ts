...
function getEl(opts: handleOverridesOpts, field: string): HTMLFormElement {
	...
	let f = opts.formRef.current!.querySelector(
		'[name="' + pref + field + '"]',
	) as HTMLFormElement;
	return f;
}
...
function getCheckbox(
	opts: handleOverridesOpts,
	field: string,
): HTMLFormElement {
	let pref = opts.prefix || "";
	let f = opts.formRef.current!.querySelector(
		'[name="' + pref + field + '"][type=checkbox]',
	) as HTMLFormElement;
	return f;
}
...
export function attach(opts: handleOverridesOpts) {
	...
	let els = [getCheckbox(opts, opts.resName + "Override")];
...
function update(opts: handleOverridesOpts) {
	...
	for (let part of opts.partsNames) {
		...
		if (!el) {
			throw new Error(
				"element to use as source data for calc not found: " + part,
			);
		}
		let s = el.value;
		let n = Number(s);
