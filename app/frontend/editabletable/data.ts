...
export function totalGroupToString(
	defs: DefData[],
	tg: TotalGroupFlags,
): TotalGroupString {
...
export function totalGroupFromString(
	defs: DefData[],
	s: TotalGroupString,
): TotalGroupFlags {
...
export function validateTotalGroup(
	itg: TotalGroupFlags,
	defs: DefData[],
): { res: TotalGroupFlags; error: Error | null } {
	...
	if (Array.isArray(itg)) {
		if (!itg.every((a) => typeof a == "string")) {
			let error = new Error(
				"Expected total group data to be an array of strings",
			);
			return { res: "invalid", error };
		}
		...
		if (!match) {
			let error = new Error(
				`Definitions in previous group selected for total no longer exist in columns defined, please select other group for total. Selected group total keys: ${itg}. Available keys: ${defKeys}`,
			);
			return { res: "invalid", error };
		}
	}
...
export class DataManager {
	...
	init(
		defs: DefData[],
		cols: DataManagerCols,
		initialData: any[][],
		ids: string[],
		initialTotalGroup: TotalGroupFlags = null,
		previousUpdatesFromJson: PreviousUpdatesFromJson = {},
	) {
		...
		this.lastTempId = 0;
		{
			...
			for (let key of this.newRows.keys()) {
				let trimmed = key.startsWith(tempIdPrefix)
					? key.slice(tempIdPrefix.length)
					: null;
				if (trimmed) {
		...
		this.sortOrder = this.sortOrder.map((g) => {
			return { key: g.key, ids: g.ids.filter((id) => !this.deletes.has(id)) };
		});
	...
	hasUnsavedChanges(): boolean {
		...
		return (
			this.updates.size > 0 || this.deletes.size > 0 || this.newRows.size > 0
		);
	}
	...
	updateTotals(fieldIndex: number, newValue: number) {
		...
		this.updateField(
			this.totalsId,
			fieldIndex + this.cols.dimensions,
			newValue,
		);
	...
	applyUpdatesWithGroupKey(): Group<DataWithId>[] {
		...
		return values
			.map((g) => {
				return {
					key: g.key,
					data: g.data.filter((r) => r.id != this.totalsId),
				};
			})
...
export function applySortOrder<T extends DataWithIdBasic>(
	rows: T[],
	...
	// check that all rows are in the sortOrder array
	{
		...
		for (let row of rows) {
			if (!allIds.has(row.id)) {
				throw new Error(
					"SortOrder array is missing id, which is in rows: " + row.id,
				);
			}
		}
	}
