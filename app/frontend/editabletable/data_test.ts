...
import { DataWithIdBasic, DefData } from "~/frontend/editabletable/base";
...
describe("DataManager", () => {
	...
	describe("applySortOrder", () => {
		...
		it("basic", () => {
			...
			let res = fn(data, [{ key: "a", ids: ["id1", "id2"] }]);
			assert.deepEqual(res, [{ key: "a", data }]);
		});
		it("basic - order adjusted", () => {
			...
			let res = fn(data, [{ key: "a", ids: ["id2", "id1"] }]);
			assert.deepEqual(res, [
				{
					key: "a",
					data: [
...
describe("groupedSort", () => {
	...
	it("basic", () => {
		...
		assert.deepEqual(res, [{ key: "1", ids: ["id1", "id2", "id3"] }]);
	});
