		...
		<div className="dts-mobilemenu-lvl dts-mobilemenu-lvl1">
			<ul>
				{items.map((item, i) => (
					<li
						key={i}
						className={`${item.lvl2 && i === index ? "selected" : ""}`}
					>
						{item.link ? (
							<a href={item.link}>
							...
							<button onClick={() => handleClick(i)} className="button-link">
								...
								{item.name}
								<Icon
									className="dts-mobilemenu-expand-icon"
									icon="undp/chevron-down"
								/>
							</button>
						)}
						{item.lvl2 && i === index && (
		...
		<div className="dts-mobilemenu-lvl dts-mobilemenu-lvl2">
			<ul>
				{items.map((item, i) => (
					<li
						key={i}
						className={`${item.lvl3 && i === index ? "selected" : ""}`}
					>
						<button onClick={() => handleClick(i)} className="button-link">
							{item.name}
							<Icon
		...
		<div className="dts-mobilemenu-lvl dts-mobilemenu-lvl3">
			<ul>
				{items.map((item, i) => (
					<li
						key={i}
						className={`${item.lvl4 && i === index ? "selected" : ""}`}
					>
						<button onClick={() => handleClick(i)} className="button-link">
							{item.title}
							<Icon
