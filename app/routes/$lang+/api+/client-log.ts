...
function validateRetentionPolicy(retentionPolicy?: RetentionPolicy): boolean {
	...
	if (
		...
		(retentionPolicy.ttlDays < 1 || retentionPolicy.ttlDays > 1825)
	) {
		return false;
	}
...
function validateSessionInfo(sessionInfo?: SessionInfo): boolean {
	...
	if (!sessionInfo.sessionId || sessionInfo.sessionId.length > 100)
		return false;
...
function prepareQueryFields(payload: EnhancedLogPayload): Record<string, any> {
	...
	if (payload.tags && Object.keys(payload.tags).length > 0) {
		...
		if (payload.tags.component)
			queryFields.tagComponent = payload.tags.component;
		if (payload.tags.environment)
			queryFields.tagEnvironment = payload.tags.environment;
		...
		if (payload.retentionPolicy.ttlDays) {
			queryFields.expiresAt = new Date(
				new Date(payload.timestamp).getTime() +
					payload.retentionPolicy.ttlDays * 24 * 60 * 60 * 1000,
			).toISOString();
		}
	}
...
function enrichLogPayload(
	payload: EnhancedLogPayload,
	request: Request,
): EnhancedLogPayload {
...
export const action: ActionFunction = async ({ request }) => {
	try {
		...
		if (!["info", "warn", "error", "debug"].includes(level)) {
			return new Response(
				JSON.stringify({ success: false, error: "Invalid log level" }),
				{
					status: 400,
					headers: { "Content-Type": "application/json" },
				},
		...
		if (!validateSamplingRate(level, samplingRate)) {
			return new Response(
				JSON.stringify({ success: false, error: "Invalid sampling rate" }),
				{
					status: 400,
					headers: { "Content-Type": "application/json" },
				},
		...
		if (!validateRetentionPolicy(retentionPolicy)) {
			return new Response(
				JSON.stringify({ success: false, error: "Invalid retention policy" }),
				{
					status: 400,
					headers: { "Content-Type": "application/json" },
				},
		...
		if (!validateTags(tags)) {
			return new Response(
				JSON.stringify({ success: false, error: "Invalid tags format" }),
				{
					status: 400,
					headers: { "Content-Type": "application/json" },
				},
		...
		if (retentionPolicy?.ttlDays) {
			responseHeaders["X-Log-Retention-Days"] =
				retentionPolicy.ttlDays.toString();
