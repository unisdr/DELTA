	...
	async ({ request, userSession }: any) => {
		...
		if (fileId && effectiveUserSession.countryAccountsId) {
			try {
				...
				const { disasterEventTable } = await import("~/drizzle/schema");
				const { dr } = await import("~/db.server");
				const { eq } = await import("drizzle-orm");

				if (debug) {
					console.log("DEBUG - Checking disaster event ownership:", {
						fileId,
						userCountryAccountsId: effectiveUserSession.countryAccountsId,
					});
				}
				...
				const event = await dr
					.select({
						...
						countryAccountsId: disasterEventTable.countryAccountsId,
					})
				...
				// If event found but doesn't belong to user's tenant
					...
					event[0].countryAccountsId !== effectiveUserSession.countryAccountsId
				) {
					console.warn(
						`Access denied: User from tenant ${effectiveUserSession.countryAccountsId} attempted to access file for disaster event ${fileId} belonging to tenant ${event[0].countryAccountsId}`,
					);
					// Redirect to unauthorized page with specific reason
					const unauthorizedUrl = `/error/unauthorized?reason=access-denied`;
					return new Response(null, {
					...
					console.log(
						`DEBUG - Access granted: Disaster event ${fileId} belongs to user's tenant ${effectiveUserSession.countryAccountsId}`,
					);
				}
			} catch (error) {
