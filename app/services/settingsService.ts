...
import { CountryAccountStatus, countryAccountStatuses } from "~/drizzle/schema/countryAccounts";
...
export async function updateSettingsService(
	...
	if (isApprovedRecordsPublic === null || isApprovedRecordsPublic === undefined) {
...
export async function updateCountryAccountService(
	...
	if (!countryAccount) {
		throw new SettingsValidationError([`Country accounts id:${id} does not exist`]);
	}
	if (!Object.values(countryAccountStatuses).includes(status as CountryAccountStatus)) {
		throw new SettingsValidationError([`Status: ${status} is not a valid value`]);
	}
