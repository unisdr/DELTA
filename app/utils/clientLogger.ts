...
class ClientLogger {
	...
	constructor(componentName: string = "", defaultContext: LogContext = {}) {
		...
		this.endpoint = `${baseUrl}/en/api/client-log`;
	...
	withContext(context: LogContext): ClientLogger {
		return new ClientLogger(this.componentName, {
			...this.defaultContext,
			...context,
		});
	}
	...
	private shouldSample(level: LogLevel): boolean {
		...
		if (
			this.logCount < this.samplingConfig.adaptiveThreshold &&
			timeSinceReset > 30000
		) {
	...
	private async sendWithRetry(
		...
		if (
			...
			try {
				...
				if (process.env.NODE_ENV === "development") {
					console.warn(
						"[Logger] sendBeacon failed, falling back to fetch:",
						beaconError,
					);
				}
			}
