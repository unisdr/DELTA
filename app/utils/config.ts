 ...
 */
import { SSOAzureB2C as interfaceSSOAzureB2C } from "~/utils/ssoauzeb2c";
...
function _configSsoAzureB2CLoginRedirectURLAdmin(): string {
	const value =
		process.env.SSO_AZURE_B2C_USERFLOW_LOGIN_ADMIN_REDIRECT_URL || "";
	return value;
}
...
function _configAuthSupported(): string[] {
	...
	valueArray.forEach(function (item) {
		if ((authAllowedArray.indexOf(item) !== -1) == false) {
			console.log(
				"Authentication configuration (.env): " + item + " is invalid.",
			);
		} else {
