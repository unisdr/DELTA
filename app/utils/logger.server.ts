...
const validateApiKey = (apiKey: string | undefined): string | null => {
	...
	if (apiKey.length < 10) {
		console.error(
			"REMOTE_LOG_API_KEY too short - should be at least 10 characters",
		);
		return null;
	}
...
class RemoteLogTransport extends Transport {
	...
	constructor(options: { endpoint: string; apiKey: string }) {
		...
		console.log(
			`Remote logging initialized to ${options.endpoint} with key ${maskApiKey(options.apiKey)}`,
		);
	}
	...
	private async flushBuffer() {
		...
		try {
			...
			if (!response.ok) {
				...
				console.error(
					`Failed to send logs to remote endpoint: ${response.statusText} (${response.status})`,
				);
				console.error(
					`Endpoint: ${this.endpoint}, Key: ${maskApiKey(this.apiKey)}`,
				);
...
const createFileTransport = (
	logType: "access" | "error" | "general" = "general",
) => {
	const filename =
		logType === "access"
	...
	const transport = new winston.transports.Console({
		...
		format: winston.format.combine(
			...
			winston.format.printf(
				({ level, message, timestamp, module, requestId, instanceId }) => {
...
const createLogger = (
	...
	if (ENABLE_REMOTE_LOGGING && REMOTE_LOG_ENDPOINT && REMOTE_LOG_API_KEY) {
		try {
			...
			console.error(
				"Failed to initialize remote logging:",
				error instanceof Error ? error.message : String(error),
			);
	...
	logger.exportLogs = async (startDate?: Date, endDate?: Date) => {
		...
		try {
			const logFiles = fs
				.readdirSync(LOG_DIR)
				.filter((file) => file.includes("dts-") && file.endsWith(".log"));
...
export const getLoggerHealthCheck = () => {
	return {
		...
		remoteEndpoint: REMOTE_LOG_ENDPOINT
			? new URL(REMOTE_LOG_ENDPOINT).origin
			: null, // Only show domain
...
const cleanup = () => {
	...
	if (ENABLE_REMOTE_LOGGING) {
		const transports = [appLogger, accessLogger, errorLogger].flatMap(
			(logger) =>
