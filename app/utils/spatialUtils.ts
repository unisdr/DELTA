...
export function convertTurfPolygon(
	...
	if (polygonGeoJSON.geometry.type === "Polygon") {
		...
		if (coordinates[0].length < 4) {
			console.warn(
				"⚠️ Not enough points for a valid polygon, returning LineString.",
			);
...
export function convertCircleToTurfPolygon(
...
export function validateCoordinateRanges(geoJson: any): {
	valid: boolean;
	error?: string;
} {
	if (!geoJson) {
...
export function checkShapeAgainstDivisions(
	...
	try {
		for (const division of divisions) {
			...
			try {
				...
				console.warn(
					`Error creating polygon for division ${division.name}:`,
					e,
				);
			}
		}
	...
	try {
		if (divisionPolygons.length === 1) {
			...
			mergedDivisionPolygon =
				combined.features.length === 1
					? (combined.features[0] as Feature<Polygon | MultiPolygon>)
					: null;
		}
	} catch (e) {
	...
	try {
		...
		if (shapeGeoJSON.geometry.type === "Polygon") {
			try {
				...
				if (
					booleanWithin(shapeGeoJSON as Feature<Polygon>, mergedDivisionPolygon)
				) {
					return "";
				}
 ...
 */
export function isValidSpatialFootprint(
	value: unknown,
	tableName?: string,
): value is {
	id: string;
		...
		value.every(
			(
				...
				((item.map_option === "Geographic level" &&
					...
					(tableName === undefined ||
						!["damages", "losses", "disruption"].includes(tableName))) ||
